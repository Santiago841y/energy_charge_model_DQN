function [NextObs,Reward,IsDone,LoggedSignals] = myStepFunction5(Action,LoggedSignals)
...
for event=1:events;
    for jj=1:rows
        ...
        elseif chrate(jj)>0  && active_ev(jj)==0 % not correct
            ...
            Reward=((sum(r5)-r5(jj))/jj+r5(jj))/rows;
                        Break=1;
        ...
        %r2(jj)=chrate(jj)*active_ev(jj)/Agg_max ; %%check service is applied makes the service attractive to be available
            ...
            r6(jj)=Table(jj,event,3)/delivered_energy(jj,departure_time-start_0+1);
...
if sum(active_ev)~=0
    r1t=(count1-count2)/sum(active_ev);

    if sum(chrate)>32*sum(active_ev) && sum(chrate)<aggregated_current
        r4t= (32*sum(active_ev)-sum(chrate))/(32*sum(active_ev)) % not to give more than required
...
Reward=(0.5*r1t++0.25*r4t+0.25*r6t)/rows;
...
IsDone = NextObs(1,1)+1 > time_all(3) || Reward==-2 || Break==1; % reached end of table no more steps can be processed
% Get reward.
